function createProductCard(product) {
    ...
    productCard.innerHTML = `
        <div class="product-image w-full md:h-30 bg-white rounded-2xl grid items-center justify-center">
            ...
            <button 
                class="add-to-cart w-26 cursor-pointer z-10 hidden bg-amber-300" data-id="${product.id}">
                <span class=" text-sm font-medium" >Add to Cart</span>
            </button>

            <div class="quantity-controls hidden justify-between rounded-md w-20 overflow-hidden border border-gray-400 ">
                <button class="btn-plus"><img src="assets/plus.png" alt="Plus" class="w-5 h-5 cursor-pointer pt-0.5 bg-gray-200"/> </button>
                <input type="number" class="text-sm font-medium w-8 text-center" data-id=${product.id} min="1" value="1">
